...
\sec Current state
Currently, the IMR is operating a demonstrator of autonomous warehouse robots. The demonstrator is part of the {\oldRepo}. The version currently used is hard to set up, supports only a proprietary format, and creates additional friction in the workflow. Moreover, the demonstrator output in text-only format, making it generally unappealing. It enables only basic functionality for the simulations, which mainly include:
