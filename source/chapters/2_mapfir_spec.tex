...
The solver is a standalone {\cmake} application and the visualizer is based on {\of} and is built using {\makefile}. This split is necessary as described in {~\ref[choosing_gui]}. The general workflow is to run the solver on a problem set as described in {~\ref[mapfir_solver_input]}, creating output in format described in {~\ref[mapfir_solver_output_format]} and visualize the solution using the visualizer {~\ref[mapfir_visualizer]}.\br
