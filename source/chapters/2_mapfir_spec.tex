...
\label[mapfir_solver]
\sec The solver
This section aims to describe and analyze the input and output formats used by the solver. This formatting will be important when migrating our application from \oldFormat.
...
\endtt
The output format is as illustrated above with the important information being the number of agents on row 2, followed by starts coordinates on row 13 in format ($x_1$, $y_1$), ($x_2$, $y_2$)... ($x_{number of agents}$, $y_{number of agents}$),  where x and y are the respective coordinates for each agent identified by the index of its coordinates on the row. Followed by goals in the same format. Following are [makespan] lines each representing a single discrete timestep in the simulation, where each line has the coordinates of each agent in the above-mentioned format.
