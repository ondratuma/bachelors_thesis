...
Commands starting with robots: are connecting directly to robots as defined in "/robot_definitions.txt" and performing actions using the defined credentials. \br The :update script command updates the server and robots, respectively, from git while using the supplied ssh keys. It also automatically fixes file permissions required to run and build the software. The ssh keys are first automatically copied to each device using scp. \br
