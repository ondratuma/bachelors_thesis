...
The commands starting with server: are connecting to the server through ssh and performing actions remotely. \br
Commands starting with robots: are connecting directly to robots as defined in "/robot_definitions.txt" and performing actions using the defined credentials. \br The :update script command updates the server and robots, respectively, from git while using the supplied ssh keys, that are first automatically copied to each device. \br
