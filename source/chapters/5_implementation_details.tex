...
Most of the setting can be changed on the admin panel on the left side of the application window.\br
The admin panel allows the user to disable individual robots, which is useful, if some of the robots battery died, but the remaining robots are still good enough for demonstration purposes. The system then does not send new commands to the disabled robots. The options may be reset even during demonstration run, so this option might by used to consistently test the synchronization algorithms, by making a robot blocked, while still being part of the execution plan. Below is the option to change the position provider for the positions showing in user interface. This option does not override the robot's internal positioning system. The default option is Robot, as the vicon startup time is not insignificant, the cameras have limited live span and for the majority of demonstration runs the feedback from robots give the exact same information within an acceptable margin of error.
\vfil \break
\
...
\line{\hsize=.3333333333\hsize \vtop{%
   ...
   \picw=3.5cm \cinspic {../assets/images/new_ui_adg_11.png}
   \caption/f Krok 10
\par}\vtop{%
   \clabel[adg_step_11]{Krok 11}
   \picw=3.5cm \cinspic {../assets/images/new_ui_adg_12.png}
   \caption/f Krok 11
\par}\vtop{%
   \clabel[adg_step_12]{Krok 12}
   \picw=3.5cm \cinspic {../assets/images/new_ui_adg_13.png}
   \caption/f Krok 12
\par}}
\endinsert
\midinsert
\line{\hsize=.3333333333\hsize \vtop{%
   ...
   \picw=3.5cm \cinspic {../assets/images/new_ui_adg_14.png}
   \caption/f Krok 13
\par}\vtop{%
   \clabel[adg_step_14]{Krok 14}
   \picw=3.5cm \cinspic {../assets/images/new_ui_adg_15.png}
   \caption/f Krok 15
\par}\vtop{%
   \clabel[adg_step_15]{Krok 15}
   \picw=3.5cm \cinspic {../assets/images/new_ui_adg_16.png}
   \caption/f Krok 16
\par}}
\endinsert