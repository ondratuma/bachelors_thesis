...
The execution of the demonstration should be guided by the synchronization strategy (Section {\ref[synchronization_strategy]}) currently selected.\br\br
The server code resides in the "{projectRoot}/FleetControl/modules/manager" directory. The only other code it touches should be the {Shared resources} further described in Section ~\ref[shared_resources].\br
...
\begitems \style x
    ...
    * sharing current Robot positions
...
\par}}
\endinsert
Most of the settings can be changed in the admin panel (Firgure {~\ref[fleet_control_admin_panel]}) on the left side of the application window.\br
...
The second currently implemented approach is to construct an action dependency graph \cite[adgPaper] of the nodes and ensure the correct execution order for actions on given nodes. The ADG is represented by an array of time steps for each robots, with each time step having the reference to its dependencies. The construction consists of iterating through the discreet time steps and for each time step and robot combination saving the the previous steps that end in the node that this step should end in. When a timeStep for a robot is executed, it is removed from the dependencies of other robots, potentially unblocking further actions. Step is only allowed, when all of its dependencies have been resolved. It greatly increases the efficiency, allowing robots to move until they break the dependency graph. In the original paper, a replanning algorithm is also introduced, but it was not implemented here for demonstration purposes, as changing the plan during execution would be quite contrary to the idea of the demonstrator, where we are demonstrating the execution plan itself. It might, however, be quite an interesting demonstration on its own. \br
\vfil \break
\secc Demonstration
...
The pink robot continues to wait until the next node in its desired path is free (from the ADG perspective) as displayed in Figure {~\ref[adg_step_7]}. This means that every robot that planned to travel through the node before this one already did so. The inverse situation, where the green robot is blocked by the pink robot, can be seen on Figures {~\ref[adg_step_12]} and {~\ref[adg_step_13]}. The rest of the demonstration does not need to avoid any collisions; it is therefore not included in this example. The robots will just return to their initial positions without any unexpected behavior.
