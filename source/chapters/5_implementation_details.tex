...
The red robot continues to wait until the next node in its desired path is free (from the ADG perspective) {~\ref[adg_step_7]}. This means that every robot that planned to travel through the node before this one already did so. The inverse situation, where robot number three is blocked by robot number one, can be seen on {~\ref[adg_step_12]}, {~\ref[adg_step_13]}. The rest of the demonstration does not need to avoid any collisions; it is therefore not included in this example. The robots will just return to their initial positions without any unexpected behavior.
