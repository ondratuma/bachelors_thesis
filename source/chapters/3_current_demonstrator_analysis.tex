...
\sec Input format
All input and output files used in the original project are XML based. The XML format has numerous advantages for advances use-cases, that need more metadata, but at the same time is hard to read by human operators and its parsing, as currently implemented, is overly complicated.
\secc Map file
