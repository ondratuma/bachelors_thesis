...
\secc{output format}
\mapfirOutputFormat 
The output format is as illustrated on ~\ref[mapfir_output_format] with the important information being number of agents on row 2, followed by starts on row 13 in format (x1, y1), (x2, y2)... (x[number of agents], y300[number of agents]),  where x and y are the respective coordinates for each agent identified by the index of its coordinates on the row
